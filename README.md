...
| Flag | Significado |
|---|---|
| `-p <pessoa>` | Pessoa (nome ou `person_id`). No `add`, pode repetir para criar a tarefa para várias pessoas de uma vez. |
...
|---|---|---|---|
| `add` | `-p`, `-m`, uma periodicidade | `-g`, `-t` | Cria a tarefa para cada pessoa em `-p`. Com `-g`, as tarefas criadas ficam vinculadas no grupo. |
...
admin add -p João -p Maria -m "lavar a louça" -d -g cozinha
